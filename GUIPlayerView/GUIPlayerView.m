...
@property (assign, nonatomic) BOOL reachedMaxBuffering;
@property (assign, nonatomic) BOOL playergotStalled;
...
@synthesize activityIndicator, progressTimer, controllersTimer, seeking, fullscreen, defaultFrame , startedBuffering;
...
//      NSLog(@"Current Value: %f",current);
//      NSLog(@"TotalDuration: %f", [self availableDuration]);
      
    [progressIndicator setSecondaryValue:([self availableDuration] / duration)];
    
      /*kavya added below code changes */
      Float64 Totaldur = CMTimeGetSeconds(player.currentItem.duration);
      //      NSLog(@"Total Duration of the video : %f",Totaldur) ;
      //      NSLog(@"Buffered Duration : %f" , [self availableDuration]);
      
      //      NSLog(@"here is :%f" , Totaldur/2);
      
      // [self availableDuration] --> Buffered Data in seconds
      // Totaldur                 --> Entire Video Duration in seconds
      
//      if ([self availableDuration] >= Totaldur/2 && !_reachedMaxBuffering)
//      {
//          
//      }
      
      if (_playergotStalled)
      {
          if ([self availableDuration] >= Totaldur)
          {
              _playergotStalled=NO;
              NSLog(@"Reached end of buffering");
              NSLog(@"Player started Playing");
              if ([delegate respondsToSelector:@selector(bufferingCompletedandReadytoPlay)])
              {
                  startedBuffering=NO;
                  [delegate bufferingCompletedandReadytoPlay];
              }
      ...
      else if ([self availableDuration] >= Totaldur/2.5 && !_reachedMaxBuffering)
      {
          _reachedMaxBuffering=YES;
          NSLog(@"Reached 40 percentage of buffering");
          NSLog(@"Player started Playing");
          //          [player play];
...
//    [player replaceCurrentItemWithPlayerItem:currentItem];
  ...
  [asset loadValuesAsynchronouslyForKeys:keys completionHandler:^{
    currentItem = [AVPlayerItem playerItemWithAsset:asset];//kavya commented below two lines
    [player replaceCurrentItemWithPlayerItem:currentItem];
    
    if (playAutomatically) {
      dispatch_sync(dispatch_get_main_queue(), ^{
//        [self play];//Kavya commented play
          [self startBuffering];
      });
    }
  }];
  
  [player setAllowsExternalPlayback:YES];//kavya commented it
  ...
  frame.origin = CGPointZero;
  [playerLayer setFrame:frame];
    playerLayer.bounds=main;
    
  [self bringSubviewToFront:controllersView];
  
//  [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];//Kavya commented it
  
  [player addObserver:self forKeyPath:@"rate" options:0 context:nil];
  [currentItem addObserver:self forKeyPath:@"status" options:0 context:nil];
  
    /*Kavya added below 2lines */
    [currentItem addObserver:self forKeyPath:@"playbackBufferEmpty" options:0 context:nil];
...
- (void)play {
 ...
 /* progressTimer = [NSTimer scheduledTimerWithTimeInterval:0.1f
                                                 ...
                                                 userInfo:nil
                                                  repeats:YES];*/ //kavya made changes
}

-(void)startBuffering
{
    startedBuffering=YES;
    progressTimer = [NSTimer scheduledTimerWithTimeInterval:0.1f
                                                     target:self
                                                   selector:@selector(refreshProgressIndicator)
                                                   userInfo:nil
...
{
    _playergotStalled=YES;//Kavya made changes
    [player pause];
  //[self togglePlay:playButton];//kavya commented this
